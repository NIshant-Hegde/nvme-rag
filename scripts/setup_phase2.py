    ...
    try:
        ...
        for model in recommended_models:
            if model in model_names:
                ...
                logger.warning(f"⚠️ Recommended model not found: {model}")
...
def run_quick_test():
    ...
    try:
        ...
        if health["status"] == "healthy":
            ...
            logger.warning(f"⚠️ Ollama health check failed: {health.get('error', 'Unknown')}")
...
def main():
    ...
    if ollama_available and not run_quick_test():
        logger.warning("⚠️ Quick test failed, but setup may still be functional")
    ...
    if ollama_available:
        ...
        logger.info("⚠️ Setup complete, but Ollama needs to be installed")
        logger.info("\nTo complete setup:")
